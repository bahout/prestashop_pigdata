...
class PigRecoa extends Module
{
    ...
    function __construct()
    {
        ...
        $this->displayName = $this->l('Pig Recommendation Engine Position A');
    ...
    function install()
    {
        if (!parent::install()
            ...
            OR !$this->setConfig('box_col_title', $this->l('Nous vous recommandons également'))
            OR !$this->setConfig('box_foo_title', $this->l('Nous vous recommandons également'))
            ...
            OR !$this->setConfig('image_size', 'auto')
            OR !$this->setConfig('desc_size', '12px')
            OR !$this->setConfig('desc_width', '319px')
            OR !$this->setConfig('price_size', '20px')
            OR !$this->setConfig('price_color', '#f0f0f0')
            OR !$this->setConfig('price_size_reg', '20px')
            OR !$this->setConfig('price_color_reg', '#f0f0f0')
    ...
    public function generateRecommendations($params)
    {
        ...
        if (!empty($pid)) {
            ...
            $pdb = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
				SELECT i.id_image, p.id_product, pl.link_rewrite, pl.name, cl.link_rewrite AS category_rewrite
    ...
    public function hookHeader()
    {
        ...
        if (_PS_VERSION_ >= 1.5) {
			//Tools::addCSS(($this->_path) . 'pigreco' . $this->refP . '.css', 'all');
            //$this->context->controller->addJS(($this->_path) . 'js/custom' . $this->refP . '.php');
            if ($statDone == false) {
                $this->context->controller->addJS(($this->_path) . 'js/pig.js');
            }
        } elseif(_PS_VERSION_ >= 1.4) {
			Tools::addCSS(($this->_path) . 'pigreco' . $this->refP . '.css', 'all');
            //Tools::addJS(($this->_path) . 'js/custom' . $this->refP . '.php');
            Tools::addJS(($this->_path) . 'js/pig.js');
        }
		else {
			return $this->display(__FILE__, $this->name.'pigfootertemplate-teddy.tpl');
		}

        Console::show($statDone, Console::INFO);

        if ($statDone == false) {
        $output = $this->creatCustomCssAndJs();
        }
        return $output;
    ...
    {
        if ($this->generateRecommendations()) {
            return $this->display(__FILE__, 'pigcolumntemplate.tpl');
        }
    }
    ...
    {
        if ($this->generateRecommendations($params)) {
            return $this->display(__FILE__, 'pigfootertemplate-teddy.tpl');
        }
    }
    ...
    private static function genDataRow($p)
    {
        ...
        if (empty($p)) {
            ...
            if (_PS_VERSION_ >= 1.5) {
                ...
                //pas propre... mais bon
                $cats = array_map('pigRecoa::toFormat', $catTree);
